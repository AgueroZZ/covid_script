...
# Function to plot Canadian province maps stratified by age, sex and wave
plot_canada_province_map <- function(result, age_group, sex_group) {
  
  for (variant in c("initial1", "initial2", "alpha", "delta", "omicron1", "omicron2")) {
    
    # Filter data for the specific wave and age group
    result_age_wave <- result %>% filter(wave == variant & age == age_group & sex == sex_group)
    ...
    # Plotting
    pdf(file = paste0("maps/Canada_map_", age_group, "_", sex_group, "_", variant, ".pdf"), width = 10, height = 10)
...
plot_canada_province_map(model_result_all, '0-44', "Males")
plot_canada_province_map(model_result_all, '0-44', "Females")

plot_canada_province_map(model_result_all, '45-64', "Males")
plot_canada_province_map(model_result_all, '65-84', "Males")
plot_canada_province_map(model_result_all, 'over 85', "Males")
plot_canada_province_map(model_result_all, '45-64', "Females")
plot_canada_province_map(model_result_all, '65-84', "Females")
plot_canada_province_map(model_result_all, 'over 85', "Females")
