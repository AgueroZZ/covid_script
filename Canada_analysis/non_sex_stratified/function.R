...
fit_mod_IWP_sGP <- function(canada_death, prior_IWP, prior_sGP, prior_overdis, k_IWP = 30, k_sGP = 30, prov, Age, p = 2, a = 2*pi, m = 1, accuracy = 0.001){
  
  full_data <- canada_death %>% filter(province == prov & age == Age)
  ...
  fitted_mod_sB <- BayesGP::model_fit(formula = death ~ f(x = x, model = "IWP", order = p, sd.prior = prior_IWP, k = k_IWP, initial_location = "left") + 
                                        f(x = x, model = "sGP", a = a, k = k_sGP, sd.prior = prior_sGP, m = m, accuracy = accuracy, region = range(x_full)) +
                                        f(x = x, model = "IID", sd.prior = prior_overdis), 
                                        data = data, family = "Poisson")
  fitted_mod_sB$prov <- prov
...
pred_mortality <- function(model_list, component = "all", over_dis = F, scale = "log", refined_pred = NULL){
  ...
  if(over_dis){
    ...
    samps_over_noise <- rnorm((3000*nrow(samples)), sd = rep(samps_overdis, each = nrow(samples)))
    samps_over_noise_matrix <- matrix(samps_over_noise, nrow = nrow(samples), ncol = 3000, byrow = F)
...
pred_mortality_obs <- function(model_list, refined_pred = NULL){
  if(is.null(refined_pred)){
    refined_pred <- seq(from = min(model_list$x_full), to = max(model_list$x_full), length.out = 100)
  }
...
excess_mortality_aggregate <- function(prov, Age, model_pred, canada_death){
  full_data <- canada_death %>% filter(province == prov, Age == age)
  ...
  delta_samps <- full_data$death - model_pred$samples
...
plot_pred_series <- function(prov, Age, model_pred, canada_death, component = "all"){
  par(cex.axis=1.5, cex.lab=1.5)
  if(component == "all"){
    full_data <- canada_death %>% filter(province == prov & age == Age)
    full_data <- na.omit(full_data)
    model_pred$summary$OBS_VALUE <- full_data$death
