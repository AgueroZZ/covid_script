...
selected_sexes <- unique(canada_weekly$Sex)
...
for (prov in selected_prov) {
  for (Age in selected_ages) {
    for (sex in selected_sexes) {
      cat(prov,":",Age,":",sex, "\n")
      selected_CA <- canada_weekly %>% filter(province == prov & age == Age, Sex == sex) %>% arrange(date)
      ...
      tryCatch({
        ...
        if(file.exists(paste0("fitted_model/", prov, "_age_", Age, "_sex_", sex, ".rda"))){
          load(file = paste0("fitted_model/", prov, "_age_", Age, "_sex_", sex, ".rda"))
        }
        else{
          ...
          save(file = paste0("fitted_model/", prov, "_age_", Age, "_sex_", sex, ".rda"), model_pred)
        }
        
        pdf(file = paste0("figure/individual_prov_series/", prov, "_", Age, "_", sex, ".pdf"), height = 10, width = 10)
        plot_pred_series(prov = prov, Age = Age, model_pred = model_pred, canada_death = selected_CA)
        dev.off()
        model_result <- excess_mortality_aggregate(Age = Age, model_pred = model_pred, prov = prov, canada_death = selected_CA)
        model_result$province <- prov
        model_result$age <- Age
        model_result$sex <- sex
        model_result_all <- rbind(model_result_all, model_result)
      }, error = function(e) {
        cat("An error occurred for province:", prov, ": age group:", Age, ":", sex, "\n", e$message, "\n")
      })
    }
  }
}
