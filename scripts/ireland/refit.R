...
if (length(script_argument) != 1L) {
  stop("Unable to determine the Ireland refit script path.")
}
...
source(file.path(project_root, "ireland_analysis", "function.R"))
source(file.path(project_root, "R", "ireland_model.R"))
...
data <- read_ireland_model_input(arguments$data)
manifest <- build_ireland_manifest(data, base_seed = 20260830L)
if (nrow(manifest) != 4L) {
  stop("Expected four Ireland models; found ", nrow(manifest), ".")
}
...
metadata <- list(
  ...
  model_code_sha256 = digest::digest(
    file = file.path(project_root, "R", "ireland_model.R"),
    algo = "sha256"
  ),
  predictor_code_sha256 = digest::digest(
    file = file.path(project_root, "ireland_analysis", "function.R"),
    algo = "sha256"
  ),
  runner_code_sha256 = digest::digest(file = script_path, algo = "sha256"),
  package_provenance = ireland_package_provenance(),
  session_info = utils::sessionInfo()
)
saveRDS(metadata, file.path(output_root, "manifests", "batch_metadata.rds"))

if (manifest_only) {
  message("Wrote the four-model Ireland manifest: ", manifest_path)
...
build_cpp <- file.path(build_root, "mixed.cpp")
if (!file.copy(
  file.path(project_root, "ireland_analysis", "mixed.cpp"),
  build_cpp,
  overwrite = TRUE
)) {
  stop("Failed to stage the Ireland TMB source.")
}
TMB::compile(build_cpp)
dyn.load(TMB::dynlib(sub("\\.cpp$", "", build_cpp)))
...
if (any(status$status == "failed")) {
  ...
  stop("Ireland refit failures: ", paste(failed, collapse = ", "), ".")
}
all_complete <- vapply(manifest$model_id, function(model_id) {
  file.exists(ireland_output_paths(output_root, model_id)$complete)
}, logical(1))
if (all(all_complete)) {
  writeLines("complete", file.path(output_root, "batch_complete.flag"))
}
message(
  "Ireland refit command completed: ",
  sum(all_complete),
  "/",
  nrow(manifest),
