...
fit_mod_IWP_sGP <- function(world_death, prior_IWP, prior_sGP, prior_overdis, k_IWP = 30, k_sGP = 30, country, p = 2, a = 2*pi, m = 1, accuracy = 0.001){

  full_data <- world_death %>% filter(geo == country)
  if(country == "ES"){
    #remove the suspicious observation on 2020-12-28
...
excess_mortality_aggregate <- function(country, model_pred, world_death){
  full_data <- world_death %>% filter(geo == country)
  if(country == "ES"){
    #remove the suspicious observation on 2020-12-28
    full_data <- full_data %>% filter(date != as.Date("2020-12-28"))
...
plot_pred_series <- function(country, model_pred, world_death, component = "all"){
  ...
  if(component == "all"){
    ...
    model_pred$summary$OBS_VALUE <- full_data$OBS_VALUE
    ...
    summary_p_score$med <- p_score_samps %>% apply(MARGIN = 1, quantile, p = 0.5)
    summary_p_score$upper <- p_score_samps %>% apply(MARGIN = 1, quantile, p = 0.975)
    summary_p_score$lower <- p_score_samps %>% apply(MARGIN = 1, quantile, p = 0.025)
    plot(med~time, type = 'l', col = "blue", ylab = "P score", data = summary_p_score, ylim = c(-2,2))
    polygon(c(summary_p_score$time,rev(summary_p_score$time)),c(summary_p_score$lower,rev(summary_p_score$upper)),col = rgb(0.5, 0.5, 0.5, alpha = 0.5), border = FALSE)
    abline(v = as.Date("2020-01-01"), col = "purple", lty = "dashed")
