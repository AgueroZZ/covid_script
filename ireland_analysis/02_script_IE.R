...
m = 1 ## Use one annual harmonic because the observed data are quarterly.
k_IWP <- 100; k_sGP <- 40
prior_overdis <- list(u = 0.1, a = 0.01)
prior_PSD1 <- list(u = 0.1, a = 0.01)
prior_PSD2 <- list(u = 0.1, a = 0.01)
prior_SD1 <- OSplines:::prior_conversion_IWP(d = d1, prior = prior_PSD1, p = p)
prior_SD2 <- prior_conversion_sGP_m(d = d2, prior = prior_PSD2, a = a, m = m)

ireland_25_45 <- ireland_summary %>% filter(new_age_group == "25-44")
# model_list_25_45 <- fit_mod_IWP_sGP(quarterly_data = ireland_25_45, prior_IWP = prior_SD1, prior_sGP = prior_SD2, prior_overdis = prior_overdis, k_IWP = k_IWP, k_sGP = k_sGP, m = m)
...
ireland_45_65 <- ireland_summary %>% filter(new_age_group == "45-64")
# model_list_45_65 <- fit_mod_IWP_sGP(quarterly_data = ireland_45_65, prior_IWP = prior_SD1, prior_sGP = prior_SD2, prior_overdis = prior_overdis, k_IWP = k_IWP, k_sGP = k_sGP, m = m)
...
ireland_65_85 <- ireland_summary %>% filter(new_age_group == "65-84")
# model_list_65_85 <- fit_mod_IWP_sGP(quarterly_data = ireland_65_85, prior_IWP = prior_SD1, prior_sGP = prior_SD2, prior_overdis = prior_overdis, k_IWP = k_IWP, k_sGP = k_sGP, m = m)
...
ireland_85 <- ireland_summary %>% filter(new_age_group == "85+")
# model_list_85 <- fit_mod_IWP_sGP(quarterly_data = ireland_85, prior_IWP = prior_SD1, prior_sGP = prior_SD2, prior_overdis = prior_overdis, k_IWP = k_IWP, k_sGP = k_sGP, m = m)
