...
plot_us_state_map <- function(result_age, age_group) {
  ...
  for (variant in c("initial1", "initial2", "alpha", "delta", "omicron1", "omicron2")) {
    ...
    result_age_wave <- result_age %>% filter(wave == variant & age == age_group)
    ...
    # Plotting
    pdf(file = paste0("maps/US_map_", age_group, "_", variant, ".pdf"), width = 10, height = 10)
    
    plot(st_geometry(us_states_joined), col = theCol$plot)
    mapmisc::legendBreaks(pos = list(x = -132, y = 48), col=theCol$col, breaks = custom_labels)
...
plot_us_state_map(model_result_all, '20-39')
plot_us_state_map(model_result_all, '40-59')
plot_us_state_map(model_result_all, '60-79')
plot_us_state_map(model_result_all, 'Over 80')
