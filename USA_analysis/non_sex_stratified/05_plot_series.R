...
smoother <- function(x,y){
  smooth.spline(x = x,y = y, all.knots = TRUE, df = 0.4 * length(unique(x)))$y
}
...
for (the_state in selected_states) {
  for (the_age in selected_ages) {
    for(the_sex in selected_sexes){
      ...
      if(!file.exists(paste0("fitted_model/", the_state, "_age_", the_age, ".rda"))){
        warning(paste0("fitted_model/", the_state, "_age_", the_age, ".rda does not exist."))
        next
      }
      load(file = paste0("fitted_model/", the_state, "_age_", the_age, ".rda"))
      full_data <- USA_monthly %>% filter(age == the_age, State_full == the_state)
